...
seeds (51001--51020); the cue-pair load experiment used 10 held-out root seeds
...
stochastic derivative-free optimizer \citep{hansen2016}. The autoencoder search minimized held-out
reconstruction error, after which independent MTL searches maximized a
composite of clean and selectively degraded recall for each plasticity rule.
...
\ref{fig:degradation}A--D used latent and output gains of 25. The
evolution-informed analyses in Fig.~\ref{fig:degradation}E,F used the selected
values $\beta_{\mathrm{latent}}=3.912$ and
$\beta_{\mathrm{out}}=25.333$. Linear encoder and decoder biases were disabled.
...
configuration, the compatibility experiment
...
experiment and 41.8 in the track experiments. Figure~\ref{fig:degradation}E,F
...
$m\in\{0.1,0.3,0.5,0.7,1,1.5,2,3,9\}$, it evaluated
\begin{equation}
\Delta J_{rjm}=
J_r(\theta_{r,-j}^*,m\theta_{r,j}^*)-J_r(\theta_r^*),
\label{eq:local-sensitivity}
\end{equation}
using the same composite objective and three development seeds as the MTL
search. Fan-in and $K_{\mathrm{CA3}}$ were rounded to integers. Values outside
...
out-of-development-set uncertainty.
