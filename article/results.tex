...
initialized autoencoders were pretrained on sparse EC patterns and then frozen.
...
cosine of 0.804 (95\% confidence interval [CI], 0.783--0.825), whereas a fixed
coordinate permutation reduced it to 0.125 (0.114--0.135). The paired
aligned-minus-permuted difference was 0.679 (0.655--0.704). The same coordinate
dependence appeared under the error-driven rule: aligned and permuted means
were 0.514 (0.493--0.535) and 0.066 (0.050--0.082), respectively, giving a
paired difference of 0.448 (0.418--0.477). Permuting the decoder together with
the IS restored the aligned mean exactly for each rule. Random-content recall
remained low (0.189 for the instructive-driven rule and 0.092 for the
error-driven rule), as did the shared
no-plasticity control (0.191). The matched-decoder restoration is an algebraic
...
basis, not on information content alone. The higher one-shot aligned recall
under the instructive-driven rule is also consistent with its full instructed write,
whereas the error-driven rule makes a bounded residual correction under the
...
\begin{figure}[!htbp]
    ...
    reconfiguration.} \textbf{(A)} Output--target cosine in aligned,
    ...
    boundaries. For each plasticity rule, panels B, E, and F used an identical
    EC--CA3 projection in the exchange and no-swap schedules.}
...
independent masks removed 0, 25, 50, 75, or 90\% of eligible units. Each
fraction used 12 masks per seed and 20 paired root seeds. The principal control
...
at the endpoint, 0.803--0.938) and from 1.000 to 0.829 under the error-driven
rule (0.766--0.893;
Fig.~\ref{fig:degradation}A). The corresponding endpoint changes were
...
baseline and 0.723 at 90\% dropout; endpoint change $-0.013$, $-0.029$ to
...
instructive-driven rule (endpoint
...
development seeds. The objective equally weighted clean recall, recall after
85\% MEC-like degradation, and recall after 85\% LEC-like degradation; these
development evaluations were not treated as experimental replicates. A
preceding autoencoder search selected
$K_{\mathrm{CA1}}=5$, $\beta_{\mathrm{latent}}=3.912$, and
$\beta_{\mathrm{out}}=25.333$. The independently selected MTL configurations
were sparse for the instructive-driven rule (EC--CA3 fan-in 1,
$K_{\mathrm{CA3}}=1$, $\beta_{\mathrm{CA3}}=260.956$,
$\beta_{\mathrm{CA1}}=5.000$, and $\alpha=0.00890$) and more distributed for
the error-driven rule (fan-in 36, $K_{\mathrm{CA3}}=4$,
$\beta_{\mathrm{CA3}}=49.614$, $\beta_{\mathrm{CA1}}=61.913$, and
$\alpha=0.02714$). Their development-set composite scores were 0.723 and
0.838, respectively. Because both parameter vectors were selected on the same
...
(Fig.~\ref{fig:degradation}E). The composite score changed smoothly near most
selected values but was not uniformly insensitive. Reducing
$\beta_{\mathrm{CA3}}$ tenfold caused the largest decrease for both rules
($-0.246$ for instructive-driven and $-0.391$ for error-driven), and extreme
changes in error-driven $\alpha$, $\beta_{\mathrm{CA1}}$, or
$K_{\mathrm{CA3}}$ were also detrimental. Small positive differences at a few
tested points were at most 0.004, consistent with finite search resolution
rather than proof of a unique optimum. The analysis therefore supports a local
...
$1.000\pm0.000$ at one pair to $0.223\pm0.029$ at 90 pairs (mean $\pm$ SEM),
with degraded recall declining from $1.000\pm0.000$ to $0.073\pm0.012$.
Error-driven recall was non-monotonic: clean accuracy was
$0.900\pm0.100$ at one pair, $0.630\pm0.054$ at 10 pairs, and
$0.751\pm0.043$ at 90 pairs; corresponding degraded values were
$0.700\pm0.117$, $0.158\pm0.023$, and $0.232\pm0.015$. Thus the two selected
regimes exhibited different load-dependent behavior. The late improvement of
the error-driven curve is consistent with repeated exposure providing a richer
calibration set for residual correction, but the present experiment does not
distinguish that account from sampling, order, or regularization effects.
...
\begin{figure}[!htbp]
    ...
    rounded and model-valid bounds were enforced; values are means over the
    three development seeds used by the search. \textbf{(F)} Exact recall of
    ordered cue pairs as the number stored increased from 1 to 90. Color
    denotes the rule; open solid curves use intact probes and filled dashed
