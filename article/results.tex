...
We evaluated two plasticity rules under matched inputs and parameters
...
whereas the error-driven rule makes a bounded residual correction at the same
learning rate.
...
\begin{figure}[!htbp]
    ...
    for the error-driven rule (left) and instructive-driven rule (right),
    ...
    similarity for the error-driven rule in the cue-exchange schedule and an
    exact-wiring no-swap control. Lap zero is the first post-exchange lap;
    ...
    boundaries. Panels B, E, and F were generated with the identical EC--CA3
    projection in the two schedules and both plasticity rules.}
...
\begin{figure}[!htbp]
    ...
    degradation.} Each panel shows the instructive-driven rule on the left and
    error-driven rule on the right. \textbf{(A)} Cue-identity accuracy during
