...
adaptation evolution strategy (CMA-ES) used to select autoencoder and
rule-specific MTL operating points was an engineering procedure, not a proposed
neural learning mechanism. The development-set fitness combined clean recall
...
different operating regimes: a nearly one-hot CA3-like key for the
instructive-driven rule and a more distributed key with steeper CA1 recall for
the error-driven rule. The sensitivity and load panels consequently compare
...
often preserved much of the composite score, whereas reducing the CA3 gain and
several extreme error-driven perturbations were strongly detrimental. This is
not a structural-identifiability analysis: parameters were varied one at a
time, only three development seeds were used, and integer rounding or clipping
created plateaus. The selected values should therefore be read as reproducible
...
non-monotonic error-driven load curve further motivates factorial tests that
hold total writes and cue-identity coverage fixed while varying the number of
stored pairs; such controls could distinguish residual calibration from serial
order and regularization effects. Experimentally, the broad prediction is
