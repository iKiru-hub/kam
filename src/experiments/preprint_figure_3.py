...
def degrade(EC_values: np.ndarray, fraction: float, modality: str,
            rng: np.random.Generator) -> tuple[np.ndarray, np.ndarray]:
    """ Zero a fixed subset of MEC or LEC coordinates for one whole probe
    lap. The mask is chosen across units, not independently at every
    track position.
...
def cue_accuracy(EC_recall: np.ndarray, cue_patterns: np.ndarray,
    ...
    similarity /= np.maximum(np.linalg.norm(lec, axis=1)[:, None] * \
            np.linalg.norm(cue_patterns, axis=1)[None, :], 1e-12)
...
def position_accuracy(EC_recall: np.ndarray, EC_target: np.ndarray) -> float:
    ...
    similarity /= np.maximum(np.linalg.norm(recalled, axis=1)[:, None] * \
            np.linalg.norm(reference, axis=1)[None, :], 1e-12)
...
def prepare_seed(seed: int, settings: dict) -> dict:
    ...
    EC_validation = cue_track(track["validation_laps"],
                              track,
                              [[0, 1]] * track["validation_laps"], seed_value(seed, 2))
    autoencoder, encoder_mse = train_autoencoder(EC_clean_laps.reshape(-1, track["size"]),
                                                 EC_validation.reshape(-1, track["size"]),
                                                 settings, seed_value(seed, 3))
    return {
...
def run_rule(seed: int, prepared: dict, settings: dict,
    ...
    shape = (len(degradation["key_modes"]), len(degradation["fractions"]),
             degradation["masks_per_fraction"])
    values = {
            ...
            for mask_index in range(degradation["masks_per_fraction"]):
                ...
                for modality in ("lec", "mec"):
                    rng = np.random.default_rng(np.random.SeedSequence([
                                seed, mode_index, fraction_index, mask_index, 0 if modality == "lec" else 1]))
                    ...
                    if modality == "lec":
                        ...
                        measurements = {
                            "lec_cue_accuracy": cue_accuracy(EC_recall,
                                                             prepared["cue_patterns"],
                                                             track["cue_positions"]),
                            "lec_mec_cosine": float(row_cosine(EC_recall[:, :track["size"] // 2],
                                                               EC_target[:, :track["size"] // 2]).mean()),
                        }
                    else:
                        ...
                        measurements = {
                            ...
                            "mec_cue_accuracy": cue_accuracy(EC_recall,
                                                             prepared["cue_patterns"],
                                                             track["cue_positions"]),
                        }
...
def plot_degradation_panel(values: np.ndarray, fractions: np.ndarray, modes: list[str],
    title: str, ylabel: str, chance: float | None = None,):
    ...
    for mode_index, mode in enumerate(modes):
        ...
        axis.plot(fractions, mean, marker="o", color=colors.get(mode, "0.2"),
                  label=f"{mode} CA3 key", linewidth=2)
        axis.fill_between(fractions, mean - sem, mean + sem,
                          color=colors.get(mode, "0.2"), alpha=0.16)
...
def build_panels(arrays: dict[str, np.ndarray], output: Path, display_rule: str) -> None:
    ...
    panel_specs = (
        ("plot_2_a", "lec_cue_accuracy", "LEC degradation: cue recall",
         "Cue identity accuracy", 0.5),
        ("plot_2_b", "lec_mec_cosine", "LEC degradation: spatial output",
         "MEC output–target cosine", None),
        ("plot_2_c", "mec_position_accuracy", "MEC degradation: spatial recall",
         "MEC nearest-position accuracy", 1.0 / 50.0),
        ("plot_2_d", "mec_cue_accuracy", "MEC degradation: cue recall",
         "Cue identity accuracy", 0.5),
    )
    for rule_index, rule in enumerate(rules):
        rule_label = RULE_LABELS.get(rule, rule)
        for name, metric, title, ylabel, chance in panel_specs:
            figure = plot_degradation_panel(arrays[metric][rule_index], fractions,
                                            modes, f"{title} ({rule_label})",
                                            ylabel, chance)
