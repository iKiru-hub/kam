...
def _main():
    ...
    logger("MTL evolution sensitivity to the ´horizon` parameter")

    num = 10
    variable = np.linspace(5, 128, num).astype(int)

    logs = {"f": [], "p": []}
    for i, v in tqdm_enumerate(variable):

        results = mee.mtlsearch(
    ...
    for i, ax in enumerate(axs):
        ...
        if i == 0:
            ...
            ax.set_ylabel(f"{mee.MTL_PARAMETER_NAMES[i-1]}")
