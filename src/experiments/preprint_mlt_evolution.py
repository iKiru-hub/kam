...
For each candidate we store clean cue-track laps and calculate three scores:
...
def evaluate_candidate_detailed(
    ...
    for item in prepared:
        ...
        clean_fidelity, clean_position, clean_cue = recall_scores(
            clean_recall, item["target"], item["cue_patterns"], track["cue_positions"]
        )
        ...
        for mask_index in range(settings["masks_per_modality"]):
            ...
            _, mec_position, mec_cue = recall_scores(
                mec_recall, item["target"], item["cue_patterns"], track["cue_positions"]
            )
            ...
            _, _, lec_cue = recall_scores(
                lec_recall, item["target"], item["cue_patterns"], track["cue_positions"]
            )
